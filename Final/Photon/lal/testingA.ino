...
int i = 0;
char publishString1[60];
/*void myHandler("to serial", data)
{
  i++;
  Serial.print(i);
  Serial.print(event);
...
{
  pinMode(A5, OUTPUT);
  //Spark.subscribe("to serial", myHandler("to serial", data));
  //Spark.subscribe("to serial2", myHandler2);
  Serial.begin(9600);
}
void loop()
{
  if (Serial.available())
  {
    ...
    sprintf(publishString1,"lala%c",inByte);
